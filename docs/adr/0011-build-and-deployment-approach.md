...
We produce an **OCI image for the backend with Spring's `spring-boot:build-image` goal** (buildpacks, no Dockerfile)
and keep **Postgres for the dev loop in `stack/docker-compose.yml`**.

- **Backend image** — `mvn -pl service/app spring-boot:build-image` builds `miravelo/app:<version>`
  (the `spring-boot-maven-plugin` `<image><name>` in `service/app/pom.xml`, JVM pinned via `BP_JVM_VERSION=21`).
...
- **Positive:** `spring-boot:build-image` produces a runnable container for the app with no Dockerfile to
